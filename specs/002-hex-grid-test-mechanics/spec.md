...
- Q: How should cell interactions be triggered (open, mark, toggle ground truth, re-cover)? → A: Click opens, Shift+click marks as filled, Option+click toggles ground truth (empty↔filled), Shift+Option+click re-covers a cell.
...
1. **Given** the app is launched with a predefined test grid selected, **When** the grid loads, **Then** flat-top hexagonal cells are rendered in the correct offset-column positions in a fully revealed state — filled cells shown as blue with flower clues, empty cells shown as dark with neighbor clues, missing positions as grey outlines.
...
4. **Given** the grid is displayed, **When** line clues are present, **Then** line clue values appear along the corresponding hex axes (vertical, upper-left-to-lower-right, upper-right-to-lower-left).

---

### User Story 2 - Interact with Cells in Developer Sandbox Mode (Priority: P2)

As a developer, I want to interact with cells beyond normal game rules — opening, marking, re-covering, and toggling ground truth — so that I can experiment with clue computation and game mechanics without being constrained by normal play flow.
...
**Independent Test**: Can be tested by launching a grid and performing all four interaction types on cells, verifying each produces the expected state change and that clues update in response.
...
2. **Given** a covered cell with ground truth FILLED, **When** the user clicks it, **Then** the cell is revealed as filled (blue), and the mistake counter increments (the player incorrectly tried to open a filled cell).
3. **Given** a covered cell, **When** the user Shift+clicks it, **Then** the cell is marked as filled (blue). If the cell's ground truth is EMPTY, the mistake counter increments.
4. **Given** any cell (covered, open, or marked), **When** the user Option+clicks it, **Then** the cell's ground truth toggles between empty and filled, and all affected clues (neighbor, flower, line) recompute and update on the grid immediately.
5. **Given** an open or marked cell, **When** the user Shift+Option+clicks it, **Then** the cell returns to the covered state.
6. **Given** a cell whose ground truth was toggled, **When** examining neighboring cells' clues, **Then** all neighbor clues, flower clues, and line clues reflect the updated ground truth.
...
1. **Given** a grid with neighbor clues displayed, **When** contiguity hints are enabled (default), **Then** clues show `{n}` for contiguous groups and `-n-` for discontiguous groups where applicable.
2. **Given** contiguity hints are enabled, **When** the user toggles them off, **Then** all clues display as plain numbers only (no curly braces or dashes).
...
3. **Given** the user selects a test grid, **When** the grid loads, **Then** the grid state is fully reset (no carried-over state from the previous grid).
...
1. **Given** a grid is displayed and some cells have been interacted with, **When** the user triggers a restart, **Then** all cells return to the fully revealed initial state (filled cells as blue, empty cells as dark with clues).
2. **Given** the user restarts the grid, **When** the grid reloads, **Then** any mistake counter or progress indicator resets to zero.
...
1. **Given** the user triggers random grid generation, **When** the grid is created, **Then** a hex grid of configurable dimensions is generated with cells randomly assigned as filled or empty.
...
- **FR-002**: System MUST support four cell visual states: covered (orange), marked filled (blue), open empty (dark), and missing hex (grey outline).
- **FR-003**: System MUST display neighbor clues inside open empty cells, supporting plain number, `{n}` contiguous, `-n-` discontiguous, and `?` (no clue) notations.
- **FR-004**: System MUST display flower clues on marked filled cells showing the count of filled cells within a 2-hex radius.
- **FR-005**: System MUST display line clues along the three hex axes (vertical, upper-left-to-lower-right, upper-right-to-lower-left).
...
- **FR-008**: System MUST provide a restart action that resets the current grid to its initial state.
- **FR-009**: System MUST provide a random grid generator that creates a hex grid with cells randomly assigned as filled or empty.
...
- **FR-012**: System MUST correctly compute line clues for any given filled/empty cell arrangement.
...
- **FR-017**: Clicking a covered cell MUST open it. If the cell's ground truth is empty, it reveals as open empty with its neighbor clue. If the cell's ground truth is filled, it is a mistake: the mistake counter increments and the cell reveals as filled (blue).
- **FR-018**: Shift+clicking a covered cell MUST mark it as filled (blue). If the cell's ground truth is empty, it is a mistake: the mistake counter increments.
- **FR-019**: Option+clicking any cell MUST toggle its ground truth between empty and filled, and immediately recompute and update all affected clues (neighbor, flower, and line).
- **FR-020**: Shift+Option+clicking an open or marked cell MUST return it to the covered state.
- **FR-021**: System MUST provide a toggle control for contiguity hint notation. When enabled, clues display `{n}` (contiguous) and `-n-` (discontiguous) where applicable. When disabled, all clues display as plain numbers.
...
- **LineClue**: A clue associated with a specific axis direction and position, indicating the count of filled cells along that line.
...
- **SC-007**: All four cell interactions (open, mark, toggle ground truth, re-cover) produce the correct state change with no perceptible delay.
...
- Cell interaction operates in a developer sandbox mode that intentionally allows actions outside normal game rules (re-covering cells, toggling ground truth). This is not meant to replicate final game interaction constraints.
